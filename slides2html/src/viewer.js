...
function doAction(el) {
  switch (el.action) {
    ...
    case 'overlay':
      if (el.slide === -2 && el.media) openImageOverlay(MEDIA(el.media));
      else if (typeof el.slide === 'number' && SLIDES[el.slide]) openSlideOverlay(el.slide);
      break;
    case 'panel': {
...
function actionFields(el) {
  ...
  if (el.action === 'overlay') {
    ...
    h += '</select></label><p class="muted">Le lecteur reste sur la page : ' +
...
function renderProps() {
  ...
  if (el.type === 'zone') {
    ...
    SLIDES.forEach(function (t, i) {
      h += opt(i, 'Diapo ' + (i + 1) + (t.hidden ? ' (cachée)' : ''),
               typeof el.slide === 'number' ? el.slide : -1);
    });
    h += '</select></label>';
    ...
    (el.list || []).forEach(function (idx, i) {
      h += '<div class="row2"><input type="text" value="Diapo ' + (idx + 1) +
    ...
    SLIDES.forEach(function (t, i) {
      h += '<option value="' + i + '">Diapo ' + (i + 1) + (t.hidden ? ' (cachée)' : '') + '</option>';
    });
    h += '</select></label>';
    if ((el.list || []).length > 1)
...
/* ============================ lecteur vidéo ============================ */
/* Ouvrir une diapo par-dessus la page courante : on ne change pas de page,
   Échap ou un clic à côté referme et on retrouve exactement où on était. */
function openSlideOverlay(idx) {
  var lb = $('lb');
  lb.innerHTML = '';
